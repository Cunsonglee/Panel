...
def load_data():
    ...
    # 数据清洗
    df['País'] = df['País'].astype(str).str.strip().replace('nan', 'Desconocido')
    df['Producto'] = df['Producto'].astype(str).str.strip().replace('nan', None)
    df['Estado_País'] = df['Estado_País'].astype(str).str.strip().replace('nan', 'Sin Estado')
    df['Estado_Producto'] = df['Estado_Producto'].astype(str).str.strip().replace('nan', 'Sin Estado')
...
if menu == "Paises":
    ...
    # 筛选器排除无效项
    all_countries = sorted([c for c in df_raw['País'].unique() if c != 'Desconocido'])
    all_p_status = sorted(list(df_raw['Estado_País'].unique()))
    ...
    stats = df_raw.groupby(['País', 'ISO3', 'Estado_País']).apply(lambda x: pd.Series({
        'Activos': ((x['Estado_Producto'].str.lower() == 'activo') & (x['Producto'].notna())).sum(),
        'Inactivos': ((x['Estado_Producto'].str.lower() == 'inactivo') & (x['Producto'].notna())).sum()
    })).reset_index()
    ...
    m3.metric("激活产品总数", ((df_raw['Estado_Producto'].str.lower() == 'activo') & (df_raw['Producto'].notna())).sum())
    
    st.subheader("国家详细列表 (全部展示)")
    # height=None 确保不出现内部分组滚动条，直接完整显示
...
# --- 2. Productos 页面 ---
    ...
    # 只显示有产品的行
    df_prod_base = df_raw[df_raw['Producto'].notna()].copy()
    
    all_countries_p = sorted([c for c in df_prod_base['País'].unique() if c != 'Desconocido'])
    all_prod_status = sorted(list(df_prod_base['Estado_Producto'].unique()))
...
# --- 3. Resumen 页面 ---
    ...
    df_res = df_raw[df_raw['Producto'].notna()].copy()
    ...
    # 提取所有非空季度并排序
    qs_regla = df_res['Q_Regla'].dropna().unique().tolist()
    qs_comp = df_res['Q_Completo'].dropna().unique().tolist()
    all_quarters = sorted(list(set(qs_regla + qs_comp)))
    
    if not all_quarters:
        st.warning("暂无有效的季度更新数据。")
    else:
